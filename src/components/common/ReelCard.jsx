...
export default function ReelCard({
  ...
  return (
    <div className={cn("relative overflow-hidden transition-all", className)}>
      ...
      {!titleInside && (
        <div className="flex flex-col gap-1 justify-between pt-1 text-black h-[70px]">
          <h1
            className={`${
              onReelsPage ? "text-xl font-semibold" : "text-sm font-medium"
            }  `}
          >
            {item?.title}
            {item?.source && (
          ...
          <div className="flex items-center justify-between w-full">
            {!onReelsPage && (
              <Link href={`/reels`}>
                <button className="text-xsm cursor-pointer hover:underline">
                  আরও দেখুন
                </button>
              </Link>
            )}
